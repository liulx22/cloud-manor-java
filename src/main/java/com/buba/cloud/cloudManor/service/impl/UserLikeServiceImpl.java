...
public class UserLikeServiceImpl implements UserLikeService {
    ...
    public int deleUserLike(UserLike userLike) {
        ...
        if (i != 0) {//如果成功在mysql删除
            boolean resourcelike_number = redisUtils.hHasKey("resource_like_number", "resourceId_" + rresourceId);//判断该key与itm是否存在
            if (resourcelike_number = false) {//如果不存在
                redisUtils.hset("resource_like_number", "resourceId_" + rresourceId, 1);//设置初始值并为1
            } else {//如果该key与itm存在
                Integer resourceId_number = (Integer) redisUtils.hget("resource_like_number", "resourceid_" + s);//获取redis中的点赞量
                redisUtils.hset("resource_like_number", "resourceId_" + s, resourceId_number - 1);//让redis中的点赞量加一
            }
        }
        return (Integer) redisUtils.hget("resource_like_number", "resourceId_" + s);//返回redis的点赞量
    }
//获取喜欢的作品数量
    @Override
